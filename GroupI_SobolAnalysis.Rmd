...
p_wrapper = function(threshold, r, g, K, func) {
  
  params <- list(threshold = threshold, r = r, g = g, K = K)
...
all_res <- pivot_longer(allres, cols = c(max_carbon, mean_carbon), names_to = "name", values_to = "carbon")


ggplot(data = all_res) +
  geom_boxplot(aes(y = carbon, x = name, fill = name))
